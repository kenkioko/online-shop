      ...
      <div class="card-body">
        ...
        <form action="{{ $form_action }}" method="post" id="user_form">
          ...
          <div class="d-none border-top p-2" id="shop_details">
            ...
            @address_form(['address'=> $shop->address])
            @endaddress_form

            <div class="form-group">
              <label for="address_full_input">Full Address:</label>
              <textarea class="form-control" name="address_full" rows="8" cols="80" id="address_full_input"
              >@if (old('address_full')){{ old('address_full') }} @elseif ($shop){{ $shop->address->full_address }}@endif</textarea>
            </div><!-- /.form-group -->
          </div>
